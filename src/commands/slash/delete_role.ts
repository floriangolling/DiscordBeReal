...
const config: SlashCommandConfig = {
  description: 'Utilisez cette commande pour supprimer un rôle à un utilisateur',
  usage: '/delete_role',
  options: [
    {
      name: 'email',
...
const command: SlashCommand = {
  ...
  execute: async (interaction) => {
    ...
    try {
      ...
      if (!role) {
        await interaction.editReply('L\'utilisateur ne possède pas ce rôle.');
        return;
      }

      await role.destroy();

      await interaction.editReply('Rôle supprimé avec succès.');
      ...
      Logger.error(`Error while using deleting role interaction for ${login}: ${error}`);
